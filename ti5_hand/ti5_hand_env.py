...
@configclass
class Ti5HandEnvCfg(DirectRLEnvCfg):
    # env
    ...
    # reward scales
    dist_reward_scale = 1.5 # distance
    rot_reward_scale = 1.5  # rotation
...
class FrankaCabinetEnv(DirectRLEnv):
    ...
    cfg: Ti5HandEnvCfg # define sence and robot physics properties and environment properties in front

    def __init__(self, cfg: Ti5HandEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.dt = self.cfg.sim.dt * self.cfg.decimation # 1/120 * 2 = 0.02 -> 50hz?
    ...
    # 该函数用于在物理步骤之前预处理动作，将动作值限制在[-1.0, 1.0]范围内，并根据动作值更新机器人的关节目标位置 
    def _pre_physics_step(self, actions: torch.Tensor):
        self.actions = actions.clone().clamp(-1.0, 1.0)
        targets = self.robot_dof_targets + self.robot_dof_speed_scales * self.dt * self.actions * self.cfg.action_scale #  计算目标位置，根据当前目标位置、速度比例、时间步长、动作和动作比例
        self.robot_dof_targets[:] = torch.clamp(targets, self.robot_dof_lower_limits, self.robot_dof_upper_limits) #  将目标位置限制在关节下限和上限之间
